...
const pool = new Pool({
    connectionString: process.env.DATABASE_URL || 'postgresql://postgres:1234@localhost:5432/user_info'
});
...
app.get('/api/admin/data', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM user_submissions ORDER BY submission_date DESC');
